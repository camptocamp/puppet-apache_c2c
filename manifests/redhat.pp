class apache::redhat inherits apache::base {
  ...
  file { ['/usr/local/sbin/a2ensite', '/usr/local/sbin/a2dissite',
          '/usr/local/sbin/a2enmod', '/usr/local/sbin/a2dismod']:
    ensure => present,
    mode   => '0755',
    owner  => 'root',
    group  => 'root',
  ...
  $module_source = $apache::params::majdistrelease ? {
  ...
  # egrep '(^|#)LoadModule' /etc/httpd/conf/httpd.conf |
  # sed -r 's|#?(.+ (.+)_module .+)|
  # echo "\1" > mods-available/redhat5/\2.load|' | sh
